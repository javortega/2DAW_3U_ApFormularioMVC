...
public class AccionLoguear implements Accion {

	private static final String VISTA ="WEB-INF/respuesta.jsp";
	...
	boolean seguir=false;
		if(ValidarFormulario.isOk(request.getParameter("login"), request.getParameter("clave"))){
			...
			try {
				...
				this.usr=this.beanDao.getUsuario(request.getParameter("login"), request.getParameter("clave"));
				this.beanDao.close();
			} 
			catch(ExcepcionErrorConexionBaseDatos ecbd){
				this.error=ecbd;
				seguir=false;
			}catch(ExcepcionClaveIncorrecta eci){
				this.error=eci;
				seguir=false;
			}catch(ExcepcionLoginNoEncontrado elne){
				this.error=elne;
				seguir=false;
